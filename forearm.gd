...
func _draw() -> void:
	...
	var angles := [180, 270, 360, 90, 180] # quel angle pour le bras gauche
	var angles_list_size = angles.size()
	var angles_distance := [30, 30, 30, 200,30] 
	var angles_pos := []
	...
	for i in range(angles_list_size):
		...
		draw_line(angles_pos[-1], left_arm_pos + Vector2(0,50) , Color('white'), 1, false)
